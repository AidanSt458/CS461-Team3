...
Grid.prototype.updateCell = function(x, y, type, targetPlayer) {
	...
	switch (type) {
		...
		case CONST.CSS_TYPE_SUNK:
			...
			document.querySelector('.' + player + ' .grid-cell-' + x + '-' + y).style.backgroundColor = 'black';
			document.querySelector('.' + 'human-player' + ' .grid-cell-' + x + '-' + y).setAttribute("ID","done");
			document.querySelector('.' + 'computer-player' + ' .grid-cell-' + x + '-' + y).setAttribute("ID","done");
			break;
		default:
			this.cells[x][y] = CONST.TYPE_EMPTY;
			break;
...
AI.prototype.initOpeningShot = function() {
    var x = Math.floor(Math.random() * 9); // Assuming it's a 9x9 board as per the typical battleship grid
    var y = Math.floor(Math.random() * 9);
    var weight = getRandom(AI.OPEN_HIGH_MIN, AI.OPEN_HIGH_MAX); // Using the highest weight range for initial impact

    AI.OPENINGS = [{'x': x, 'y': y, 'weight': weight}];
};
...
AI.prototype.shoot = function() {
	...
	for (var x = 0; x < Game.size; x++) {
		for (var y = 0; y < Game.size; y++) {
			var theDiv = document.getElementById(x+"-"+y);
			if (theDiv != null){
			//var content = document.createTextNode(this.probGrid[x][y]);
			if (maxProbability > 1000){
				var norm = (((this.probGrid[x][y])/maxProbability)*100);
				//alert(this.probGrid[x][y]+" "+maxProbability);
				theDiv.innerHTML = norm.toFixed(0)+"%"; 
			}
			else{
				theDiv.innerHTML = this.probGrid[x][y].toFixed(0)+"%";
			}

			var farFromMax = maxProbability/this.probGrid[x][y];
			//alert(maxProbability+" "+this.probGrid[x][y]);
			
			if(farFromMax == 1) {
				theDiv.style.backgroundColor = 'green';
			}
			else if(farFromMax < 1.25) {
				theDiv.style.backgroundColor = '#FF4B00';
			}
			else if(farFromMax < 2) {
				theDiv.style.backgroundColor = '#FF7F00';
			}
			else if(farFromMax < 2.5) {
				theDiv.style.backgroundColor = '#FFAC00';
			}
			else if(farFromMax < 5) {
				theDiv.style.backgroundColor = '#FFE800';
			}
			else{
	...
	scroll(260,260);
	setTimeout(() => alert("Look below to see where th AI calculated to shoot next. The grid square it calculated is the most likely to have an enemy ship is square "+(maxProbCoords.x+1)+"-"+(maxProbCoords.y+1)+". Hit OK to continue."),0);
